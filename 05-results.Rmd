...
**Total Beers in Beer Ratings data:**

```{r, echo=FALSE}
cat(paste("No.of Beers: ", nrow(cleaned_all_beers[!is.na(cleaned_all_beers$beer_name), ])))
```

**Total Breweries in Brewery info data:**

```{r, echo=FALSE}
cat(paste("No.of Breweries: ", nrow(cleaned_all_breweries[!is.na(cleaned_all_breweries$brewery), ])))
```
...
We used [tigris](https://rdocumentation.org/packages/tigris/versions/1.5) to get spatial data for state boundaries in US and t_map for generating an interactive map to show the number of breweries across the US.

```{r, echo=FALSE, warning=FALSE, message=FALSE}
mean_rating_map <-
  inner_join(us_geo, state_beer_summary_info) %>% select(
    NAME,
    AverageRating,
    BreweryCount,
    NumRatings,
...
my_interactive_map <- tm_shape(mean_rating_map) +
  ...
  tm_layout(
    title = "Breweries Summary by State in US",
    legend.position=c("left", "bottom")
  )

my_interactive_map <-
  tmap_leaflet(my_interactive_map, add.titles = TRUE)
my_interactive_map
```
...
You can read more about beer styles using the following links

[Sour - Fruited](https://www.brewersassociation.org/edu/brewers-association-beer-style-guidelines/#:~:text=vary%20widely%20EBC)-,American%2DStyle%20Fruited%20Sour%20Ale,-Color%3A%20Can%20range)

[IPA - American](https://www.brewersassociation.org/edu/brewers-association-beer-style-guidelines/#:~:text=6%2D14%20EBC)-,American%2DStyle%20India%20Pale%20Ale,-Color%3A%20Pale%20to)

[Pale Ale - American](https://www.brewersassociation.org/edu/brewers-association-beer-style-guidelines/#:~:text=16%2D36%20EBC)-,American%2DStyle%20Pale%20Ale,-Color%3A%20Straw%20to)

[New England - IPA](https://vinepair.com/wine-blog/what-is-new-england-ipa/)

[Sour - Other](https://www.brewersassociation.org/edu/brewers-association-beer-style-guidelines/#:~:text=20%2B(40%2B%20EBC)-,American%2DStyle%20Sour%20Ale,-Color%3A%20Pale%20to)

[Double New England IPA](https://hazyandhoppy.com/when-does-an-ipa-become-a-double-ipa/)

[Porter - American](https://www.totalwine.com/beer-guide/ale/porter/american-porter#:~:text=American%20Imperial%20Porter%20is%20emerging,reaching%2010%20percent%20or%20more.)

[Porter - Other](https://www.craftbeer.com/beer-styles?family=porters)

[Blonde Ale](https://www.brewersassociation.org/edu/brewers-association-beer-style-guidelines/#:~:text=Origin%20Ale%20Styles-,Golden%20or%20Blonde%20Ale,-Color%3A%20Straw%20to)

[Brown Ale](https://en.wikipedia.org/wiki/Brown_ale)

[Stout - Imperial](https://www.craftbeer.com/styles/american-imperial-stout)

[Stout - Milk/Sweet](https://www.craftbeer.com/styles/english-style-sweet-stout-milk-stout)

[Stout - American](https://www.craftbeer.com/styles/american-stout)

[American - Lager](https://www.brewersassociation.org/edu/brewers-association-beer-style-guidelines/#:~:text=Origin%20Lager%20Styles-,American%2DStyle%20Lager,-Color%3A%20Straw%20to)

[Märzen](https://www.americancraftbeer.com/what-the-hell-is-a-maerzen/)

[Red American Amber](https://craftbeerclub.com/beer-style/american-amber--red-ale#:~:text=This%20an%20umbrella%20term%20that,from%20amber%20to%20red%20tones.&text=Ambers%20use%20the%20crystal%20malt,reddish%20Munich%20or%20Vienna%20malts.)

[Wheat - Hefeweizen](https://www.craftbeer.com/styles/german-style-hefeweizen)
...
To answer this question, we will go back to our brewery data, where it contains the rating of every brewery (calculated as the average of all beer ratings) from every state, we could easily dump all of the brewery ratings in ridgeline plot by the state to look at rating behaviors across the country. To avoid overplotting, we will only show the top 20 states with the most breweries in this analysis.
...
beer_style_production_by_year <- cleaned_all_beers %>%
  ...
  mutate(beer_style_wit_abb = paste(abbreviate(beer_style), ": ", beer_style))
```
...
ggplot(
  ...
  mapping = aes(
    alluvium = beer_style_wit_abb,
    x = year,
    stratum = factor(production_rank)
  )
) +
  geom_flow(aes(fill = beer_style_wit_abb), color = "black") +
  geom_stratum(aes(fill = beer_style_wit_abb)) +
  geom_text(stat = "alluvium",
            ...
            size = 3) +
  scale_fill_manual(name = "Beer Styles", values = beer_style_palette) +
  guides(y = "none", fill = guide_legend(ncol = 3)) +
  theme_wsj(color = "gray") +
  ggtitle("Top 10 Beer Styles\nBy No.of New Beers Added in Each Style")
```
