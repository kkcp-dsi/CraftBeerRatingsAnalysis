...
beer_style_palette <- distinctColorPalette(25)
ggplot(
...
```{r, echo=FALSE}
west_east_comparison <- cleaned_all_beers %>%
...
vcd::mosaic(beer_rating_category ~ beer_abv_category + beer_ibu_category, 
            direction=c("v", "v", "h"), 
            west_east_comparison,
            highlighting_fill=brewer.pal(3, 'Reds'))
```



## Ratings for West Coast IPA and East Coast IPA

```{r, echo=FALSE}
west_east_comparison <- cleaned_all_beers %>%
  filter(state %in% c("California", "Massachusetts", "New_York", "Connecticut")) %>%
  mutate(region=ifelse(state %in% c("California", "Oregon", "Washington"), "West Coast", "East Coast")) %>%
  select(beer_style, state, region, beer_average_rating, beer_abv, beer_ibu) %>%
  drop_na() %>%
  filter(beer_style == "IPA - American")
```

```{r, echo=FALSE}
GGally::ggparcoord(west_east_comparison, 
                   columns=4:6, 
                   scale='uniminmax', 
                   alphaLines = 0.5, 
                   splineFactor = 5,
