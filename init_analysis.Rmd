...
cleaned_all_beers <- all_beers %>% 
  ...
  mutate(beer_dated=as.Date(beer_added, format = "%m/%d/%y")) %>%
  mutate(year=format(beer_dated, format="%Y"))
```
...
cleaned_all_beers %>% 
  ...
  select(beer_style, year) %>%
  group_by(beer_style, year) %>%
  summarise(freq=n()) %>%
  ungroup() %>%

  
ggplot(loans_full_schema, aes(loan_amount, reorder(loan_purpose, loan_amount, FUN = median))) + 
  geom_density_ridges() + 
  labs(title="Ridgelines of loan amount (x) by different loan purposes (y)")
...
```{r}
test <- top_most_popular_beer_styles %>%
  mutate(beer_style=fct_reorder(factor(beer_style), rn))
