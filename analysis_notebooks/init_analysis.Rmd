...
#state_beer_summary_info <- 
#  inner_join(state_avg_rating, inner_join(top_brewery_per_state, inner_join#(top_beer_per_state, top_beer_style_per_state)))


#brewery_summary_info <- inner_join(state_avg_rating, top_brewery_per_state)
#brewery_summary_info
```
...
ggplot(beer_style_production_by_year, mapping = aes(
  alluvium = beer_style,
  x = year,
  ...
  geom_text(stat = "alluvium", aes(label = beer_style_production_by_year$beer_style), size = 2) +
  #scale_fill_manual(values = brewer.pal(n = 50, name = "Set1")) +
